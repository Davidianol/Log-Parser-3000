...
**Связи, которые теоретически могли бы быть:**

| Связь                                          | Условие                                                                                                                          |
|------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| `Node -> Port`                                 | `ports.node_guid = nodes.node_guid` (уже реализовано через FK)                                                                   |
| `Node <-> Node` (физический линк)              | `port.neighbor_node_guid` указывает на соседний узел,  два порта с совпадающими `neighbor_node_guid` / `port_num` образуют ребро |
| `Switch -> Host` (коммутатор обслуживает хост) | `port_state = 4` (Active) на порту свитча, ведущем к `node_type = 1`                                                             |
| `Node -> Subnet`                               | Группировка по `gid_prfx` (GID-префикс) — узлы с одинаковым префиксом в одной подсети                                            |
